...
bool ContinuousMeasurementApiHandler::canHandle(
    AsyncWebServerRequest *request) const {
  if (request->method() != HTTP_GET) return false;
  char url_buf[AsyncWebServerRequest::URL_BUF_SIZE];
  const auto url = request->url_to(url_buf);
  return url == "/continuous/start" ||
         url == "/continuous/stop" ||
         url == "/continuous/status";
}

void ContinuousMeasurementApiHandler::handleRequest(
  ...
  } else if (url == "/continuous/stop") {
    this->handle_stop_(request);
  } else if (url == "/continuous/status") {
...
bool ContinuousMeasurementApiHandler::parse_interval_(
    AsyncWebServerRequest *request, uint32_t &interval_ms,
...
void ContinuousMeasurementApiHandler::handle_start_(
    AsyncWebServerRequest *request) {
  if (this->controller_ == nullptr) {
    this->send_snapshot_(request, 500, "error",
                         "continuous_measurement_unavailable");
    return;
  }
  ...
  if (interval_ms < 200 || interval_ms > 10000) {
    this->send_snapshot_(request, 400, "error",
                         "interval_ms_out_of_range");
    return;
  }
  ...
  if (!this->controller_->start(interval_ms)) {
    ...
    const int response_code =
        (start_error == "calibration_required" ||
         start_error == "precise_roi_unsupported")
            ? 409
            : 500;
    this->send_snapshot_(
        request, response_code, "error",
        start_error.empty() ? "continuous_start_failed"
                            : start_error.c_str());
    return;
  }
...
void ContinuousMeasurementApiHandler::handle_stop_(
    AsyncWebServerRequest *request) {
  if (this->controller_ == nullptr) {
    this->send_snapshot_(request, 500, "error",
                         "continuous_measurement_unavailable");
...
void ContinuousMeasurementApiHandler::handle_status_(
    AsyncWebServerRequest *request) const {
  if (this->controller_ == nullptr) {
    this->send_snapshot_(request, 500, "error",
                         "continuous_measurement_unavailable");
...
void ContinuousMeasurementApiHandler::send_snapshot_(
    AsyncWebServerRequest *request, int response_code,
  ...
  if (this->controller_ != nullptr) {
    ...
    json += "\"";
    json += ",\"interval_ms\":" +
            std::to_string(this->controller_->interval_ms());
    json += ",\"cycle_count\":" +
            std::to_string(this->controller_->cycle_count());
    json += ",\"target_found_count\":" +
            std::to_string(this->controller_->target_found_count());
    json += ",\"valid_measurement_count\":" +
            std::to_string(this->controller_->valid_measurement_count());
    json += ",\"target_found\":";
    json += this->controller_->last_cycle_target_found() ? "true" : "false";
    json += ",\"measurement_valid\":";
    json += this->controller_->last_cycle_measurement_valid()
                ? "true"
                : "false";

    const uint32_t capture_ms = this->controller_->last_capture_ms();
    const uint32_t decode_ms = this->controller_->last_decode_ms();
    ...
    const uint64_t processing_sum =
        static_cast<uint64_t>(capture_ms) + decode_ms +
        detect_ms + compute_ms;
    const uint32_t processing_ms =
        processing_sum > 0xFFFFFFFFULL
            ? 0xFFFFFFFFU
            : static_cast<uint32_t>(processing_sum);
    const uint32_t orchestration_ms =
        cycle_ms > processing_ms ? cycle_ms - processing_ms : 0U;
    ...
    json += ",\"orchestration_ms\":" +
            std::to_string(orchestration_ms);
    ...
    if (this->tracking_controller_ != nullptr) {
      ...
      json += "\"";
      json += ",\"target_locked\":";
      json += this->tracking_controller_->target_locked()
                  ? "true"
                  : "false";
      json += ",\"lost_count\":" +
              std::to_string(
      ...
      const auto *viewport_controller =
          this->tracking_controller_->viewport_controller();
      if (viewport_controller != nullptr) {
        ...
        json += ",\"horizontal_mirror\":";
        json += viewport.horizontal_mirror ? "true" : "false";
        json += ",\"vertical_flip\":";
        json += viewport.vertical_flip ? "true" : "false";
        json += ",\"capture_mode\":\"" +
                this->controller_->last_cycle_viewport_mode() + "\"";
        json += ",\"capture_roi_x\":" +
                std::to_string(
                    this->controller_->last_cycle_viewport_x());
        json += ",\"capture_roi_y\":" +
                std::to_string(
                    this->controller_->last_cycle_viewport_y());
        json += ",\"capture_roi_width\":" +
                std::to_string(
                    this->controller_->last_cycle_viewport_width());
        json += ",\"capture_roi_height\":" +
                std::to_string(
                    this->controller_->last_cycle_viewport_height());
      }
      json += "}";
    }
  ...
  json += "}";
  auto *response =
      request->beginResponse(response_code, "application/json", json);

...
void ApiWsdlHandler::handleRequest(AsyncWebServerRequest *request) {
  ...
  xml += "<api name=\"geometrie-camera\" version=\"42\" style=\"REST-over-HTTP\">\n";
  ...
  xml += "    <item>GeometryMeasurementEngine V6.1 conserve les dimensions issues des droites subpixel robustes et utilise les separations locales comme controle independant pour estimer la confiance de chaque axe ; pose_z_mm reste un controle independant issu de l homographie.</item>\n";
  ...
  xml += "    <item>Distance V6.1 : profil normal echantillonne tous les 0,25 px, moyenne tangentielle sur 7 pixels, polarite imposee du bord exterieur noir, rejet adaptatif des transitions internes puis ajustement robuste des quatre droites. Largeur/hauteur restent calculees par separation des droites, methode la plus stable observee. Les separations locales entre bords opposes servent uniquement de controle de coherence et augmentent l incertitude d un axe lorsqu elles divergent. La fusion Z largeur/Z hauteur est ponderee par cette confiance.</item>\n";
  xml += "    <item>Calibration optique automatique : apres verrouillage PRECISE, le frame courant sert de baseline. La qualite est mesuree sur une ROI serree autour de la cible. 5 essais AE automatiques comparent les images sans supposer que status.aec_value/agc_gain expose les registres auto reels, puis 11 essais manuels exposition/gain parcourent les bornes et resserrent la recherche (16 prises evaluees maximum). Le score est continu et combine nettete, qualite de detection, RMS subpixel et contraste noir/blanc P10-P90, afin qu'un balayage AE sombre n'interrompe jamais la recherche manuelle. Le meilleur profil est verrouille avec AEC/AGC OFF et conserve pour les mesures suivantes.</item>\n";  xml += "    <item>Calibration focale robuste : les X echantillons fx/fy sont agreges par mediane/MAD, rejet des valeurs aberrantes puis moyenne des inliers afin qu une capture anormale ne deplace pas tout le profil.</item>\n";
  ...
  xml += "    <item>En mode continu, SEARCH/ZOOM_WIDE/ZOOM_MEDIUM/ZOOM_FINE servent uniquement au tracking et ne produisent aucune mesure geometrique ni aucun echantillon d historique. Seuls les frames PRECISE natifs 800x600 alimentent MeasurementManager. Sur PRECISE, MeasurementManager stabilise une fenetre de 5 acquisitions en filtrant separement Z largeur/Z hauteur et leurs poids avant la fusion V6. Toute transition/recentrage remet cette fenetre a zero.</item>\n";
  ...
  xml += "    <comment>Expose la cible detectee et le diagnostic de raffinement subpixel : subpixel_refined, subpixel_rms_px, subpixel_max_rms_px, subpixel_gradient, subpixel_width_px/subpixel_height_px ainsi que subpixel_width_sigma_px/subpixel_height_sigma_px et les gradients par axe.</comment>\n";
  xml += "    <response code=\"200\" content_type=\"application/json\"/><response code=\"500\" content_type=\"application/json\"/>\n";
  xml += "  </method>\n";
  ...
  xml += "    <comment>Retourne measurement (valeur publiee), raw_measurement (derniere mesure brute) et stabilization avec sample_count/window_size/distance_stddev_mm/distance_span_mm. measurement/raw_measurement exposent edge_v4_used/edge_v5_used pour compatibilite, edge_v6_used, apparent_width_px/apparent_height_px, leurs incertitudes et les poids de fusion largeur/hauteur. La stabilisation est active uniquement sur les acquisitions PRECISE du mode continu.</comment>\n";
  xml += "    <response code=\"200\" content_type=\"application/json\"/><response code=\"500\" content_type=\"application/json\"/>\n";
  xml += "  </method>\n";

...
void ApiWsdlHandler::handleRequest(AsyncWebServerRequest *request) {
  ...
  xml += "<api name=\"geometrie-camera\" version=\"40\" style=\"REST-over-HTTP\">\n";
  ...
  xml += "    <item>Distance V5 : profil normal echantillonne tous les 0,25 px, moyenne tangentielle sur 7 pixels, rejet adaptatif des points et ajustement robuste des quatre droites. La largeur/hauteur viennent directement des paires de droites opposees et chaque axe fournit une incertitude estimee. La distance fusionne largeur/hauteur avec des poids derives de cette incertitude. Si V5 est indisponible, le moteur conserve automatiquement le calcul precedent.</item>\n";
  ...
  xml += "    <item>En mode continu PRECISE, MeasurementManager stabilise les mesures sur une fenetre de 5 acquisitions : amorcage brut sur 2 points, puis stabilisation robuste separee de Z largeur/Z hauteur et de leurs poids de confiance avant une nouvelle fusion V5. Toute transition/recentrage de viewport remet cette fenetre a zero.</item>\n";
  ...
  xml += "    <comment>Retourne measurement (valeur publiee), raw_measurement (derniere mesure brute) et stabilization avec sample_count/window_size/distance_stddev_mm/distance_span_mm. measurement/raw_measurement exposent edge_v4_used pour compatibilite, edge_v5_used, apparent_width_px/apparent_height_px, leurs incertitudes et les poids de fusion largeur/hauteur. La stabilisation est active uniquement sur les acquisitions PRECISE du mode continu.</comment>\n";
  xml += "    <response code=\"200\" content_type=\"application/json\"/><response code=\"500\" content_type=\"application/json\"/>\n";
  xml += "  </method>\n";

...
CameraSettingsApiHandler::CameraSettingsApiHandler(CameraSettingsController *controller) : controller_(controller) {}
...
void CameraSettingsApiHandler::handleRequest(AsyncWebServerRequest *request) {
  ...
  if (url == "/api/camera/settings") {
    this->handle_status_(request);
    return;
  }

  if (url == "/api/camera/settings/set") {
...
bool CameraSettingsApiHandler::parse_bool_param_(AsyncWebServerRequest *request, const char *name, bool &value,
                                                 bool &present, std::string &error) const {
  int parsed = 0;
  if (!this->parse_int_param_(request, name, parsed, present, error)) {
    return false;
  }
  if (!present) {
    return true;
...
void CameraSettingsApiHandler::handle_set_(AsyncWebServerRequest *request) {
  ...
  int aec_value = 0;
  int agc_gain = 0;
  bool exposure_ctrl = false;
  bool gain_ctrl = false;
  bool has_brightness = false;
  bool has_contrast = false;
  bool has_exposure_ctrl = false;
  bool has_ae_level = false;
  bool has_aec_value = false;
  bool has_gain_ctrl = false;
  bool has_agc_gain = false;
  std::string error;
  ...
  if (!has_brightness && !has_contrast && !has_exposure_ctrl && !has_ae_level && !has_aec_value &&
      !has_gain_ctrl && !has_agc_gain) {
